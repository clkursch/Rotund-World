...
public class patch_Player
{
	...
	*/
    ...
    public static int SlugcatStats_NourishmentOfObjectEaten(On.SlugcatStats.orig_NourishmentOfObjectEaten orig, SlugcatStats.Name slugcatIndex, IPlayerEdible eatenobject)
    {
        ...
        if (eatenobject is SlimeMold slime && ((ModManager.DLCShared && (eatenobject as PhysicalObject).abstractPhysicalObject.type == DLCSharedEnums.AbstractObjectType.Seed) || slime.JellyfishMode))
        {
    ...
    public static float GetTileSizeMod(Creature self, Vector2 posMod, IntVector2 inputVec, int sizeMod, bool inPipe, bool submerged, bool showLogs)
    {
        ...
        if (showLogs)
            Debug.Log("-----TILE SIZE MODIFIER!: " + tileSeed + "+" + tileSizeMod + (shortcutFlag ? (" -SHORTCUT TYPE!: " + self.room.shortcutData(myShortcutTile).shortCutType) : " ") + " " + self.room.GetTilePosition(self.bodyChunks[1].pos) + " " + posMod + " -COIN:" + coinFlip + " ROOM:" + self.room.roomSettings.name + " HARD? " + BPOptions.hardMode.Value + " WETHZD: " + wetHazard + " BONUS: " + self.GetBelly().stuckInShortcut);
    ...
    public static void Player_ObjectEaten(On.Player.orig_ObjectEaten orig, Player self, IPlayerEdible edible)
    {
        ...
        if (edible is SlimeMold slime && (!ModManager.DLCShared || slime.abstractPhysicalObject.type != DLCSharedEnums.AbstractObjectType.Seed) && !slime.JellyfishMode)
        {
	...
	*/
    ...
    public static void BP_GrabUpdate2(Player self, bool eu)
    {
        ...
        if (BPOptions.hudHints.Value && !BellyPlus.smearHintGiven && (BellyPlus.backFoodHint2 || !BPOptions.backFoodStorage.Value) && self.room.game.cameras[0].hud != null
            ...
            && (GetOverstuffed(self) >= 5 - BellyPlus.BPODifficulty()) //(GetChubValue(self) >= 3 && UnityEngine.Random.value <= 1f)
	...
	*/
    ...
    public static void CheckStuckage(Player self, bool eu)
    {
        ...
        if (self.GetBelly().stuckStrain > 0)
        {
            ...
            if (BPOptions.debugLogs.Value && UnityEngine.Random.value < 0.01f)
        ...
        //HARDMODE STRUGGLE AT HIGH SIZES
        if (BellyPlus.BPODifficulty() >= 3f && self.GetBelly().isStuck)
        {
            int tryThresh = self.GetBelly().beingPushed > 0 ? 8 : 0;
            float progress =  (30 * tileSize) - self.GetBelly().stuckStrain; //USE 30*TILESIZE TO GET THE UNCAPPED SQUEEZETHRESHOLD
            if (self.GetBelly().boostStrain <= tryThresh)
            {
                self.GetBelly().loosenProg = Mathf.Max(0f, self.GetBelly().loosenProg - (Mathf.Sqrt(Mathf.Max(0f, progress - 300))) / 30000f);
            }
                
            if (BPOptions.debugLogs.Value && UnityEngine.Random.value < 0.05f)
                Debug.Log("-----TILETIGHTNESS DEBUG!: " + progress + " - " + self.GetBelly().loosenProg);
        }
    }
    ...
    public static void BPUUpdatePass5(Player self)
    {
        ...
        //HOLD JMP TO STRAIN SOLO?
        else if ((self.input[0].jmp && self.input[1].jmp && (self.GetBelly().boostCounter < 12)) && (!self.lungsExhausted || self.submerged) && (self.GetBelly().isStuck && matchingStuckDir))
        {
            //self.GetBelly().boostStrain < 10;

            ObjGainBoostStrain(self, 0, (UnityEngine.Random.value < 0.2 ? 2 : 1), 32); //18

            self.GetBelly().corridorExhaustion += 2;

            float boostAmnt = 3.5f; //18
            float loosenAmnt = boostAmnt / 4f;
            self.GetBelly().stuckStrain += boostAmnt * ((GetLivingPlayers(self) > 1) ? 0.6f : 1f);
            float loosenMod = (GetLivingPlayers(self) > 1) ? 3000f : 2000f;
            ObjGainLoosenProg(self, (loosenAmnt * (ObjIsSlick(self) ? 3f : 1f)) / loosenMod);

            if (self.GetBelly().boostCounter <= 0 && self.GetBelly().boostStrain > 20)
            {
                MakeStrainSparks(self, 1);
            }
        }
    ...
    public static void BPUUpdatePass7(Player self, bool eu)
    {
		...
		*/
        ...
        //REINTRODUCE SOME TERRAIN ENCASEMENT SAFETY MEASURES
        bool encased = self.IsTileSolid(1, 0, 0) && self.IsTileSolid(0, 0, 0);
        if (encased && self.Consious && self.enteringShortCut == null && !IsStuck(self))
        {
            if (self.input[0].jmp && !self.input[1].jmp) //IF WE JUMP, TP
                GetNearestShortcut(self, 125, false);

            if (self.input[0].IntVec == new IntVector2(0, -1)) //IF WE'RE HOLDING DOWN, TP TO THE NEAREST DOWNWARDS FACING PIPE
                GetNearestShortcut(self, 75, true);
        }

    }


    public static void GetNearestShortcut(Creature creature, float dist, bool downOnly)
    {
        if (creature.room == null)
            return;

        ShortcutData? closestShortcut = null;
        float closestDist = 9999;

        foreach (ShortcutData shortCut in creature.room.shortcuts)
        {
            if (shortCut.shortCutType == ShortcutData.Type.RoomExit || shortCut.shortCutType == ShortcutData.Type.Normal)
            {
                float mydist = Vector2.Distance(creature.room.MiddleOfTile(shortCut.startCoord), creature.mainBodyChunk.pos);
                if (mydist < closestDist && (downOnly == false || creature.room.ShorcutEntranceHoleDirection(shortCut.StartTile) == new IntVector2(0, 1)))
                {
                    closestDist = mydist;
        ...
        if (closestDist < dist && closestShortcut != null)
        {
            creature.enteringShortCut = closestShortcut.Value.StartTile;
            Debug.Log("PIPE SAFETY WARP: " + closestDist);
        }
    }
